...
use evm_rpc_types::{
    ConsensusStrategy, JsonRpcError, MultiRpcResult, ProviderError, RpcConfig, RpcError,
...
impl EthRpcClient {
    ...
    pub fn multi_request<'a>(
        &self,
        method: RpcMethod,
        params: Option<&'a Value>,
    ) -> MultiRpcRequest<Option<&'a Value>, RawJson> {
        MultiRpcRequest::new(
            self.providers(),
            method,
            params,
...
pub struct MultiRpcRequest<'a, Params, Output> {
    providers: &'a BTreeSet<RpcService>,
    method: RpcMethod,
    params: Params,
...
impl<'a, Params, Output> MultiRpcRequest<'a, Params, Output> {
    pub fn new(
        providers: &'a BTreeSet<RpcService>,
        method: RpcMethod,
        params: Params,
        response_size_estimate: ResponseSizeEstimate,
        reduction_strategy: ReductionStrategy,
    ) -> MultiRpcRequest<'a, Params, Output> {
        MultiRpcRequest {
            providers,
            method,
            params,
...
impl<Params, Output> MultiRpcRequest<'_, Params, Output> {
        ...
        Output: Debug + DeserializeOwned + HttpResponsePayload,
    {
        ...
        let client =
            http_client(MetricRpcMethod::from(self.method.clone()), true).map_result(|r| match r?
                .into_body()
                .into_result()
            {
                Ok(value) => Ok(value),
