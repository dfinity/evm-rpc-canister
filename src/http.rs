                                ...
                                match error {
                                    IcError::CallRejected(error) => {
                                        ...
                                        log!(
                                            ...
                                            req_data.request_id,
                                            error.raw_reject_code(),
                                            error.reject_message(),
                                        );
                                    }
                                    IcError::InsufficientLiquidCycleBalance(_) | IcError::CallPerformFailed(_) | IcError::CandidDecodeFailed(_) => {
                                        todo!()
                                    }
                                }
                            }
...
impl From<HttpClientError> for RpcError {
    fn from(error: HttpClientError) -> Self {
        match error {
            HttpClientError::IcError(IcError::CallRejected(error)) => {
                RpcError::HttpOutcallError(HttpOutcallError::IcError {
                    code: LegacyRejectionCode::from(error.raw_reject_code()),
                    message: error.reject_message().to_string(),
                })
            }
            HttpClientError::IcError(
