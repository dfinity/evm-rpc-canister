    ...
    Request: MaxResponseBytesRequestExtension + Clone,
{
    ...
    fn retry(
        &mut self,
        req: &mut Request,
        result: &mut Result<Response, BoxError>,
    ) -> Option<Self::Future> {
        ...
        match result {
            Ok(_) => None,
            Err(e) => {
                if let Some(ic_error) = e.downcast_ref::<IcError>() {
